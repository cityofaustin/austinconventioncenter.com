...
namespace :deploy do
  task :acc do
    puts "Deploying to ACC Production"
#     exec "SITE=acc S3_BUCKET=ACC-PROD-CHANGEME s3_website push --site=_site/acc"
  end

  task :pec do
    puts "Deploying to PEC Production"
#     exec "SITE=pec S3_BUCKET=PEC-PROD-CHANGEME s3_website push --site=_site/pec"
  end

  task :acc_staging do
    puts "Deploying to ACC Staging"
    exec "SITE=acc_staging S3_BUCKET=staging.austinconventioncenter.com s3_website push --site=_site/acc_staging"
  end

  task :pec_staging do
    puts "Deploying to PEC Staging"
...
#     ---
#
#     Disable deploys from master for now
#     ---
    puts "||||||||||||||||||||||||||||||||||||||"
    puts "Skipping deploys from master temporarily"
#     exec "parallel bundle exec rake deploy:{} ::: acc pec"
  elsif ENV["CIRCLE_BRANCH"] == "staging"
    puts "||||||||||||||||||||||||||||||||||||||"
    puts "Deploying acc_staging and pec_staging"
