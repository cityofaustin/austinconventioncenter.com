(function($){'use strict';$.jCarousel.plugin('jcarouselSwipe',{_options:{perSwipe:1,draggable:true,method:'scroll'},_init:function(){var self=this;this.carousel().on('jcarousel:reloadend',function(){self._reload();});},_create:function(){this._instance=this.carousel().data('jcarousel');this._instance._element.css('touch-action',(!this._instance.vertical?'pan-y':'pan-x'));this._carouselOffset=this.carousel().offset()[this._instance.lt]+parseInt(this.carousel().css((!this._instance.vertical?'border-left-width':'border-top-width')))+parseInt(this.carousel().css((!this._instance.vertical?'padding-left':'padding-top')));this._slidesCount=this._instance.items().length;this.carousel().find('img, a').attr('draggable',false).css('user-select','none').on('dragstart',function(event){event.preventDefault()});this._destroy();if(this._instance.items().length>this._instance.fullyvisible().length){this._initGestures();}},_initGestures:function(){var self=this;var startTouch={};var currentTouch={};var started=false;var animated=false;var xKey=!this._instance.vertical?'x':'y';var yKey=!this._instance.vertical?'y':'x';var edgeLT,lastItem;var startTarget;this._element.on('touchstart.jcarouselSwipe mousedown.jcarouselSwipe',dragStart);function dragStart(event){event=event.originalEvent||event||window.event;event.preventDefault();startTouch=getTouches(event);startTarget=event.target||event.srcElement;if(self._options.draggable){$(document).on('touchmove.jcarouselSwipe mousemove.jcarouselSwipe',dragMove);}
    ...
    function dragEnd(event){event=event.originalEvent||event||window.event;currentTouch=getTouches(event);if(started||(!self._options.draggable&&Math.abs(startTouch[xKey]-currentTouch[xKey])>10)){var newTarget=self._getNewTarget(startTouch[xKey]-currentTouch[xKey]>0);newTarget=self._instance._options.wrap==='circular'?newTarget.relative:newTarget.static;if(startTarget===event.target){$(event.target).on("click.disable",function(event){event.stopImmediatePropagation();event.stopPropagation();event.preventDefault();$(event.target).off("click.disable");});}
        ...
        animated=true;self._instance[self._options.method](newTarget,function(){started=false;animated=false;if(self._instance._options.wrap!=='circular'){self._removeClones();self._instance._items=null;}});}
